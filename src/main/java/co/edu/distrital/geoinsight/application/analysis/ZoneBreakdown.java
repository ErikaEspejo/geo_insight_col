 ...
 * por atributos de distribución y las entidades. Solo para movimientos en masa
 * se incluyen distribuciones por TIPO, SUBTIPO y CLAS_MAPA (FR-010). Sin
...
public record ZoneBreakdown(
    ...
    private static Map<String, Long> distribution(List<GeoscienceEntity> entities, String attribute) {
        ...
        for (GeoscienceEntity entity : entities) {
            entity.attributeString(attribute).filter(value -> !value.isBlank())
                    .ifPresent(value -> distribution.merge(value, 1L, Long::sum));
        }
        return distribution;
    }
}
